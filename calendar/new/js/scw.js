...
//******************************************************************************
    ...
    var scwBaseYear        = scwDateNow.getFullYear()-108;
    ...
    function scwSetDefaultLanguage()
        {try
            ...
            {// English
             scwToday               = 'Сегодня:';
             scwClear               = 'Очистить';
             scwDrag                = 'нажмите, чтобы передвинуть';
             scwArrMonthNames       = ['Янв','Февр','Март','Апр','Май','Июнь',
                                       'Июль','Авг','Сен','Окт','Нояб','Дек'];
             scwArrWeekInits        = ['Вс','Пн','Вт','Ср','Чт','Пт','Сб'];
             scwInvalidDateMsg      = 'Введена неверная дата.\n';
             scwOutOfRangeMsg       = 'Введенная дата выходит за предел.';
             scwDoesNotExistMsg     = 'Введенная дата не существует.';
             scwInvalidAlert        = ['Неверная дата (',') проигнорировано.'];
             scwDateDisablingError  = ['Ошибка.'];
             scwRangeDisablingError = ['Ошибка',
                                       ' должно состоять из двух элементов.'];
            }
        };
    ...
    k = 0;    
    if((new Date(scwBaseYear, 0, 1)) - dateRestriction.low < 0)
        scwDisabledDates[k++] = [new Date(scwBaseYear, 0, 1), dateRestriction.low];
    if(dateRestriction.high - (new Date(scwBaseYear + scwDropDownYears, 0, 1)) < 0)
        scwDisabledDates[k++] = [dateRestriction.high, new Date(scwBaseYear + scwDropDownYears, 0, 1)];
    ...
    document.writeln(
        '<style type="text/css">'                                       +
            ...
            'td.scwCellsExMonth         {background-color:  #CCCCCC;'   +
                                        'color:             #666666;}'  +
            '/* Today selector */'                                      +
            'td.scwNow                  {background-color:  #CCCC9B;'   +
                                        'color:             #000000;}'  +
            '/* Clear Button */'                                        +
            ...
            'td.scwCellsExMonthDisabled {background-color:  #999999;'   +
                                        'color:             #666666;}'  +
            'td.scwNowDisabled          {background-color:  #FFFFFF;'   +
                                        'color:             #000000;}'  +
        '</style>'
                    );
...
//  Variables required by both scwShow and scwShowMonth
    ...
    function scwID(id)
        {if (document.getElementById(id)
           || (!document.getElementById(id) && document.getElementsByName(id).length==0))
