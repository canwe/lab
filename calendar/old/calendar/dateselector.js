...
var fixedY = -1;
var startAt = 1;
var crossobj, monthSelected, yearSelected, dateSelected, omonthSelected;
var oyearSelected, odateSelected, monthConstructed, yearConstructed;
var ctlToPlaceValue, ctlNow, dateFormat, nStartingYear;
var bPageLoaded = false;
var today = new Date();
var dateRestriction = {
   low: new Date(1951, 0, 14),
   high: new Date(2050, 11, 15)
};
...
if (dom)  // What is this test? Is it good?
{
...
function constructCalendar() {
   ...
   for (datePointer = 1; datePointer <= numDaysInMonth; datePointer++)
   {
      ...
      var navDate = new Date(yearSelected, monthSelected, datePointer);
      if (navDate - dateRestriction.low < 0 || dateRestriction.high - navDate < 0)
      {

         sHTML += "<span class='date-out-of-reach'>"
            + datePointer + "</span>";
         delete navDate;
         navDate = null;
      }
         ...
         sHTML += "<a class='" + sStyle + "' title=\"" + sHint + "\" href='javascript:dateSelected="
            + datePointer + ";closeCalendar();'>" + datePointer + "</a>"
      }
...
function popUpCalendar(ctl, ctl2, format) {
   ...
   if (bPageLoaded)     // Only the developer was aware of this variable
   // Therefore DateSelectorInit() should be moved to the beginning of func.
   {
      if (crossobj.visibility == "hidden") {
         ...
         {
            // use user's date
            aData = ctl2.value.split(formatChar) // parse data in input field of form
...
function DateSelectorInit() {      // init calendar caption
   if (!ns4)
   {
      if (!ie) {
         yearNow += 1900
...
function DocumentRegisterEvents() // register events for document object to hide/open calendar
{
   document.onkeypress = function hideCalender_Trap1() // ie only
   {
      if (event.keyCode == 27)
      {
         hideCalendar();
